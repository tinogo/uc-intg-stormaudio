...
class StormAudioIntegrationDriver(
    ...
    async def refresh_entity_state(self, entity_id):
        ...
        logging.getLogger(Loggers.DRIVER).debug(
            "Refreshing entity state: %s", entity_id
        )
        ...
        match self.entity_type_from_entity_id(entity_id):
            case EntityTypes.MEDIA_PLAYER.value:
                self.api.configured_entities.update_attributes(
                    entity_id, device.get_device_attributes(entity_id)
                )
            case EntityTypes.SENSOR.value:
                entity = self.api.configured_entities.get(entity_id)
                if isinstance(entity, StormAudioSensor):
                    filtered_attributes = entity.get_sensor_attributes()

                    self.api.configured_entities.update_attributes(
                        entity_id, filtered_attributes
                    )
